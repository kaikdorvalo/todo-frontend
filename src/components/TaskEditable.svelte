...
<div class="max-w-lg bg-zinc-50 px-7 py-4 border border-slate-200 shadow-md rounded-md mb-3">
    ...
    {#if addingSubTask}
        <div>
            ...
            <div class="w-1/2">
                <button on:click={cancelEditingSubTask} class="border-2 border-red-400 text-red-400 mt-2 me-2 py-1 px-2 hover:bg-red-400 hover:text-white">Voltar</button>
                <button on:click={newSubTask} class="text-indigo-600 mt-2 py-1 px-2 hover:bg-indigo-600 hover:text-white border-2 border-indigo-600">Adicionar</button>
            </div>
        </div>
    {:else}
